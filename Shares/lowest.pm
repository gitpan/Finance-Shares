...
sub build {
	...
	my $high = lowest( $values, $o->{period}, $i );
    ...
    unless ($l->{key}) {
	my $dtype = $q->dates_by;
	$l->{key} = "lowest '$src->{key}' of last $o->{period} $period{$dtype}s";
    }
}
...
sub lowest {
    ...
    return $min < $highest_int ? $min : undef;
}
...
Finance::Shares::lowest - Show the lowest value in the last N periods
...
This module calculates the lowest value reached by some other line in the
previous N periods.
