...
sub build {
	...
	if ($percent) {
	    $cutoff = $max  - ($boundary/100) * ($max - $min);
	} else {
	...
	$b->{data}[$last]  = $cutoff;
	$b->interpolate();
	$b->{key} = "boundary for undersold '$skey'";
    }
}
...
Two examples of how to specify a undersold line, one showing the minimum
...
The number of standard deviations that are considered normal.  The undersold line
...
undersold line will go high.  Note that this is a fraction of the gradient
