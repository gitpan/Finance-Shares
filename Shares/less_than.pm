...
sub build {
    ...
    my $d1 = $first->{data};
    my $d2 = $second->{data};
    $l->{key} = "'$first->{key}' less than '$second->{key}'" unless $l->{key};
...
Finance::Shares::less_than - Compare two lines, value by value
...
The entry must have a B<function> field, C<less_than>, and a B<line> or
B<lines> field indicating two source lines.

=head3 function

Required.  Must be C<less_than>.
